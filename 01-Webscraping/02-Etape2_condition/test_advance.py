...
test_advance = pd.DataFrame(
    list(zip(title, reviews, rate)), columns=["Title", "reviews", "stars"]
)


test_advance.to_csv("../csv_final/test_advance.csv", index=False)
...
while page2 != 47:
    url = f"https://www.trustpilot.com/review/advanceamerica.net?page={page2}&stars=1&stars=2"
    response = requests.get(url)
    html = response.content
    soup = BeautifulSoup(html, "html.parser")
    for titles in soup.find_all(
...
# -------------------------------------------------------------------------------------------
df = pd.read_csv("../csv_final/advance.csv")
...
df1 = pd.read_csv("../csv_final/star_1_advance.csv")


union = pd.concat([df, df1], axis=0)


union.to_csv("../csv_final/new_csv_advance.csv", index=False)


df_new = pd.read_csv("../csv_final/new_csv_advance.csv")


dfs = df_new.drop_duplicates()

dfs.to_csv("../csv_final/01-advance.csv", index=False)
