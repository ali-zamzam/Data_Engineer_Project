...
test_cashnet = pd.DataFrame(
    list(zip(title, reviews, rate)), columns=["Title", "reviews", "stars"]
)


test_cashnet.to_csv("../csv_final/test_cashnet.csv", index=False)

# ------------------------------------------------------------------------------------------------
page2 = 2
while page2 != 37:
    url = f"https://www.trustpilot.com/review/www.cashnetusa.com?page={page2}&stars=1&stars=2"
    response = requests.get(url)
    html = response.content
    soup = BeautifulSoup(html, "html.parser")
    for titles in soup.find_all(
...
test_cashnet.to_csv("../csv_final/star_1_cashnet.csv", index=False)

# ---------------------------------------------------------------------------------------------
df = pd.read_csv("../csv_final/cashnet.csv")
...
df1 = pd.read_csv("../csv_final/star_1_cashnet.csv")

# print(df1.head)

# df2 = pd.read_csv("../csv_final/star_2_cashnet.csv")
...
union.to_csv("../csv_final/new_csv_cash.csv", index=False)


df_new = pd.read_csv("../csv_final/new_csv_cash.csv")


dfs = df_new.drop_duplicates()

dfs.to_csv("../csv_final/02-cashnet.csv", index=False)
