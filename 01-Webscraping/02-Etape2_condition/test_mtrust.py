...
millineum_trust = pd.DataFrame(
    list(zip(title, reviews, rate)), columns=["Title", "reviews", "stars"]
)

millineum_trust.head()

# to save the dataframe
millineum_trust.to_csv("../csv_final/millineum_trust.csv", index=False)

# ------------------------------------------------------------------------------------------------
page2 = 2
while page2 != 62:
    url = f"https://www.trustpilot.com/review/www.mtrustcompany.com?page={page2}&stars=1&stars=2"
    response = requests.get(url)
    html = response.content
    soup = BeautifulSoup(html, "html.parser")
    for titles in soup.find_all(
...
test_mtrust = pd.DataFrame(
    list(zip(title, reviews, rate)), columns=["Title", "reviews", "stars"]
)


test_mtrust.to_csv("../csv_final/star_1_mtrust.csv", index=False)


# -------------------------------------------------------------------------------------------
df = pd.read_csv("../csv_final/millineum_trust.csv")
...
df1 = pd.read_csv("../csv_final/star_1_mtrust.csv")


union = pd.concat([df, df1], axis=0)


union.to_csv("../csv_final/new_csv_mtrust.csv", index=False)


df_new = pd.read_csv("../csv_final/new_csv_mtrust.csv")


dfs = df_new.drop_duplicates()

dfs.to_csv("../csv_final/04-mtrust.csv", index=False)
