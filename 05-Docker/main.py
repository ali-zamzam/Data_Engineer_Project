...
tags_metadata = [
    {
        "name": "user",
        "description": "Savoir quel utilisateur est connecté en temps réel.",
    },
    {
        "name": "prediction",
    ...
    {
        "name": "delete user",
        "description": "Supprimer un utilisateur existant de la base de données",
    },
]
...
def current_user(username: str = Depends(security)):
...
@app.post("/sentiments-prediction", tags=["prediction"])
async def predict_sentiment(review: str, cleaned_review=Depends(security)):
    # clean the review
...
@app.delete("/users", tags=["delete user"])
